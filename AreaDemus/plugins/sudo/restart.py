...
from AreaDemus.misc import HAPP, SUDOERS, XCB
...
async def update_(client, message, _):
    ...
    _update_response_ = "<b>a new update is available for the bot,pushing updates now.\n\n<b><u>Updates :</u></b>\n\n"
    _final_updates_ = _update_response_ + updates
    if len(_final_updates_) > 4096:
        url = await AnonyBin(updates)
        nrs = await response.edit(
            f"<b>a new updates is available for the bot, pushing updates now.\n\n<u><b>Updates :</b></u>\n\n<a href={url}>Check Updates</a>"
        )
    else:
...
async def restart_(_, message):
    ...
    await response.edit_text(
        "Restart process started, please wait for few seconds until the bot ready.. ."
    )
    os.system(f"kill -9 {os.getpid()} && bash start")
